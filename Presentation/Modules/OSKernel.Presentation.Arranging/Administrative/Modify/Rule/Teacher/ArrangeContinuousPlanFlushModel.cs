...
{
    public class ArrangeContinuousPlanFlushModel : CommonViewModel, IInitilize
    {
        ...
        public ArrangeContinuousPlanFlushModel()
        {
        ...
        {
            this.Comments = CommonDataManager.GetAdminRuleComments(AdministrativeRuleEnum.ArrangeContinuousPlanFlush);
            ...
            if (rule != null)
            {
                rule.ContinuousPlanFlushes.ForEach(h =>
                {
                    // 1.绑定规则选中状态。
        ...
        void save(HostView host)
        {
            ...
            if (filters?.Count() > 0)
            {
                ...
                {
                    var continousPlan = new XYKernel.OS.Common.Models.Administrative.Rule.ArrangeContinousPlanRule()
                    {
                        CourseID = r.CourseID,
                        TeacherID = r.TeacherID,
                        FlushDays = r.FlushDays,
                        ClassIDs = r.Classes.Where(c => c.IsChecked)?.Select(s => s.ID)?.ToList(),
                        Weight = (int)r.Weight
                    };
                    rule.ContinuousPlanFlushes.Add(continousPlan);
                }
            }
