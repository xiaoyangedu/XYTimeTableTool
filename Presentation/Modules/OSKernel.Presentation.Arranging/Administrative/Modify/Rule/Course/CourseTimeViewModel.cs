...
namespace OSKernel.Presentation.Arranging.Administrative.Modify.Rule.Course
{
    public class CourseTimeViewModel : CommonViewModel, IInitilize
    {
        ...
        void modifyCommand(object obj)
        {
            ...
            var result = rule.CourseTimes.FirstOrDefault(a => a.CourseID.Equals(classModel.CourseID) && a.ClassID.Equals(classModel.ID));
        ...
        void save(HostView host)
        {
            ...
            if (periods != null)
            {
                ...
                {
                    var first = rule.CourseTimes.FirstOrDefault(ct => ct.CourseID.Equals(sc.CourseID) && ct.ClassID.Equals(sc.ClassID));
                    if (first != null)
                    {
