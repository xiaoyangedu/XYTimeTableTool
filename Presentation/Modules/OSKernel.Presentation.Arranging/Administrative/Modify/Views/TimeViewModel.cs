...
namespace OSKernel.Presentation.Arranging.Administrative.Modify.Views
{
    ...
    /// 课位ViewModel
    /// </summary>
    public class TimeViewModel : CommonViewModel, IInitilize
    {
        ...
        void saveCommand()
        {
            ...
            information.AppendLine("规则-课表锁定");
            information.AppendLine("规则-教师排课时间");
            information.AppendLine("规则-同时开课限制");
            information.AppendLine("规则-课程排课时间");
            //information.Append("辅修科目规避时间");
            //information.Append("主修科目优先时间及排课占比");
            //information.Append("教师在特定课位的课时限制");
            information.AppendLine();
            ...
            AdministrativeDataHelper.TimeChanged(base.LocalID, CommonDataManager);
        ...
        void es(bool isChecked)
        {
            if (isChecked)
            {
                ...
                {
                    this.ShowDialog("提示信息", "下午没课,不能添加晚自习", CustomControl.Enums.DialogSettingType.OnlyOkButton, CustomControl.Enums.DialogType.Warning);
